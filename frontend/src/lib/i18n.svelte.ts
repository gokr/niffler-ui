// UI chrome localization. Model-facing strings (tool schemas, system
// prompts, tool descriptions) deliberately stay English so model behavior
// is language-stable — only human-visible chrome text lives here.
//
// Catalog discipline: `en` is the source of truth; zh and zh-TW are typed
// as Record<DictKey, string>, so a missing or extra key in either language
// is a compile error (same contract as the desktop UIs we benchmarked).
// {placeholder} tokens are filled by t(key, vars) and must keep the same
// names across languages.
//
// Reactive via Svelte 5 runes (hence the .svelte.ts suffix): t() reads
// `current` during render, so every component calling it re-renders when
// the locale flips. The choice persists to localStorage and falls back to
// navigator.language (zh-Hant/TW/HK → Traditional, other zh → Simplified).

export type Locale = "en" | "zh" | "zh-TW";
export type DictKey = keyof typeof en;

const en = {
  "app.newSession": "+ New session",
  "app.session": "session {id}",
  "app.newSessionLabel": "new session",
  "app.connecting": "connecting…",
  "app.busConnected": "bus connected",
  "app.busUnreachable": "bus unreachable",
  "app.selectModel": "Select model",
  "app.model": "model:",
  "app.tokens": "{used} / {context} tokens",
  "app.switchLight": "Switch to light mode",
  "app.switchDark": "Switch to dark mode",
  "app.switchLanguage": "Switch language",
  "app.browserNotice": "Running in a browser — the bus bridge only exists inside the desktop shell. Launch it with",
  "app.unreachableNotice": "bus unreachable at {url} — start the harness and keep its terminal open:",
  "app.conversationHeld": "This conversation is open in Niffler {n}.",
  "app.conversationHeldOther": "This conversation is open in another UI.",
  "app.conversationHeldSwitched": "This conversation is open in Niffler {n} — started a new conversation.",
  "app.conversationHeldOtherSwitched": "This conversation is open in another UI — started a new conversation.",
  "app.startFresh": "Start a fresh conversation",
  "app.dismiss": "Dismiss",
  "app.approvalRequired": "Approval required",
  "app.approvalWaiting": "A tool call with x-harness.approval is waiting for your ok:",
  "app.moreWaiting": "+ {n} more waiting",
  "app.autoApproving": "auto-approving this session:",
  "app.enterEsc": "Enter approve · Esc deny",
  "app.deny": "Deny",
  "app.approve": "Approve",
  "app.autoApprove": "Auto approve",
  "app.autoApproveTitle": "Approve and don't ask again for this tool for the rest of this session",
  "app.continueTitle": "Keep going?",
  "app.continueBody": "The turn reached its {dimension} limit:",
  "app.continueHint": "Continue raises the limit by one step and keeps the turn running · Enter continue · Esc stop",
  "app.continueYes": "Continue",
  "app.continueNo": "Stop",
  "app.dimensionRounds": "rounds",
  "app.dimensionTokens": "tokens",
  "app.dimensionSeconds": "time",
  "sessions.newSession": "New session",
  "sessions.notStarted": "not started yet",
  "sessions.rename": "Rename",
  "sessions.delete": "Delete session",
  "sessions.confirmDelete": "Click again to delete",
  "sessions.sure": "sure?",
  "chat.placeholder": "Ask Niffler to do something…",
  "model.search": "Search models…",
  "model.customId": "custom model id",
  "model.set": "Set",
  "model.useDefault": "Use provider default",
  "model.clearsOverride": "(clears override)",
  "model.searching": "Searching…",
  "model.noneFound": "No models found",
  "model.noneFoundFor": "No models found for \"{catalog}\"",
  "model.reason": "reason",
  "model.tools": "tools",
  "model.showing": "Showing {n} of {total} — narrow your search",
  "pm.title": "Providers",
  "pm.addProvider": "Add provider",
  "pm.editProvider": "Edit provider",
  "pm.save": "Save",
  "pm.saving": "Saving…",
  "pm.edit": "Edit",
  "pm.sure": "Sure?",
  "pm.remove": "Remove",
  "pm.active": "active",
  "pm.noKey": "no key",
  "mcp.title": "MCP servers",
  "mcp.addServer": "Add server",
  "mcp.editServer": "Edit server",
  "mcp.save": "Save",
  "mcp.saving": "Validating & saving…",
  "mcp.edit": "Edit",
  "mcp.sure": "Sure?",
  "mcp.remove": "Remove",
  "mcp.refresh": "Refresh",
  "mcp.live": "running",
  "mcp.notRunning": "not running",
  "mcp.disabled": "disabled",
  "mcp.enabled": "Enabled",
  "mcp.validateNow": "Validate & start now",
  "mcp.loading": "loading…",
  "mcp.none": "No MCP servers configured yet",
  "mcp.tools": "{n} tools",
  "mcp.keepBlank": "(leave blank to keep)",
  "mcp.hint": "Tools surface as mcp_<server>_<tool> and are called through discover + invoke. Saving validates the server with one real connect — first runs of npx/uvx servers may download packages.",
  "pc.provider": "Provider",
  "pc.providerDetail": "Provider: {provider} ({source})",
  "components.direct": "direct",
  "components.seen": "seen",
  "components.demand": "demand",
  "components.internal": "internal",
  "components.noneSeen": "none seen yet",
  "components.exposureInit": "tool exposure initializes with the first turn",
  "components.noSession": "start or select a session to see tool exposure",
  "components.tools": "tools",
  "app.effort": "effort",
  "app.think": "think",
  "app.toolsDisplay": "tool",
  "app.effortTitle": "Thinking effort (applies between turns)",
  "app.thinkTitle": "Reasoning display",
  "app.toolsTitle": "Tool card display",
  "app.stop": "Stop",
  "app.stopConfirm": "Stop?",
  "app.stopping": "stopping…",
  "chat.send": "Send",
  "chat.steer": "Steer",
  "chat.sendWhileBusy": "Turn running — Enter steers it instead",
  "chat.unknownCommand": "unknown command: /{name}",
  "chat.thinkingEffort": "thinking effort: {level}",
  "chat.thinkLevel": "reasoning display: {level}",
  "chat.toolLevel": "tool card display: {level}",
  "chat.slashExec": "→ /{name} {args}",
  "chat.slashFailed": "/{name} failed: {err}",
  "chat.completing": "fetching {token}…",
  "chat.levelAuto": "auto",
  "chat.levelFull": "full",
  "chat.levelBrief": "brief",
  "chat.levelOff": "off",
  "chat.thinkingCollapsed": "▸ thinking…",
  "chat.contextAt": "context at {pct}% — will compact/trim at {trim}%",
  "chat.contextAtSoon": "context at {pct}% — will compact/trim soon",
  "chat.compacted": "context compacted — the size shown is an estimate until the next request re-measures it",
  "chat.contextTrimmed": "context trimmed — dropped {n} earlier messages",
  "chat.pruned": "tool results pruned — {bytes} bytes reclaimed",
  "chat.cacheReset": "history trimmed — provider prompt cache resets on the next request",
  "chat.compactionUnavailable": "compaction unavailable",
  "chat.compactionDeclined": "compaction declined",
  "chat.compactionInvalid": "compaction rejected",
  "chat.compactionStale": "compaction stale",
  "chat.compactionFailed": "compaction failed",
  "chat.compactionOther": "compaction {reason}",
  "status.provider": "provider",
  "status.unknown": "unknown",
  "status.unknownSource": "unknown source",
  "status.none": "none",
  "status.env": "(environment)",
  "status.global": "(global)",
  "status.session": " (session)",
  "status.ctxNone": "no context limit",
  "status.ctxEmpty": "limit {context}",
  "status.detailTitle": "Provider/model/context details",
  "status.detailProvider": "provider: {provider} ({source})",
  "status.detailModel": "model: {model}",
  "status.detailCatalog": "catalog: {catalog}",
  "status.detailContext": "context limit: {context} ({source})",
  "status.detailOutput": "output limit: {output} ({source})",
  "status.detailUsed": "context used: {used} ({pct}%)",
  "status.detailCache": "prompt cache: {read}/{prompt} ({rate}%)",
  "status.detailStrip": "model prefix stripping: on",
  "status.detailOverride": "model override: {model}",
  "help.title": "Commands:",
  "help.keys": "Ctrl+T thinking · Ctrl+E tool cards · Ctrl+G effort",
  "help.pluginTitle": "Plugin commands:",
  // Built-in slash command descriptions for /help. slash.ts is the source
  // of the command list (names, params, aliases); these keys localize the
  // descriptions only — new builtins ship English-first via slashDescription().
  "slash.components": "running components and conversation tool exposure",
  "slash.discover": "append component schemas to this conversation (tool=NAME for one tool)",
  "slash.profile": "select tool profile for new conversations; default clears",
  "slash.provider": "choose the global provider",
  "slash.model": "choose this conversation's model",
  "slash.effort": "thinking effort",
  "slash.connect": "open provider setup",
  "slash.status": "provider/model/context details",
  "slash.new": "start a new conversation",
  "slash.session": "switch conversation",
  "slash.think": "reasoning display: full, brief or off",
  "slash.tools": "tool card display: brief, full or off",
  "slash.locale": "switch the UI language",
  "slash.info": "this session's stats: model, messages, tokens, prompt cache",
  "slash.help": "show this help",
  "slash.approvals": "approval gate for this conversation: ask (default) or auto",
  "slash.limit": "this conversation's soft turn limits (rounds/tokens/seconds); clear resets them",
  // /approvals and /limit rendering (core's session status readback)
  "approvals.current": "approval gate: {mode}",
  "approvals.updated": "approval gate set: {mode}",
  "approvals.modeAsk": "ask — gated tool calls prompt you",
  "approvals.modeAuto": "auto — gated tool calls run without prompting",
  "limits.current": "soft turn limits: {limits}",
  "limits.updated": "soft turn limits: {limits} — dimensions you did not name are cleared",
  "limits.cleared": "soft turn limits cleared — only the hard budgets apply",
  "limits.none": "none",
  "limits.rounds": "{n} rounds",
  "limits.tokens": "{n} tokens",
  "limits.seconds": "{n}s",
  "limits.clearArgs": "clear takes no arguments",
  "limits.hint": "reaching one asks whether the turn may keep going",
  "controls.noSession": "no conversation selected — start one with /new or send a message first",
  // /info rendering (core's session_info)
  "info.id": "ID: {id}",
  "info.title": "Title: {title}",
  "info.model": "Model: {model} · {provider}",
  "info.effort": "Thinking: {level}",
  "info.created": "Created: {time}",
  "info.cwd": "Workspace: {cwd}",
  "info.messages": "Messages: {total} (user {user} · assistant {assistant} · tool {tool})",
  "info.context": "Context: {used} / {limit} ({pct}%)",
  "info.contextUsed": "Context: {used}",
  "info.input": "Input: {total} — cached {cached} ({rate}%) · uncached {uncached}",
  "info.output": "Output: {output}",
  "info.noSession": "no session selected — /info [id] to inspect another conversation",
} as const;

const zh: Record<DictKey, string> = {
  "app.newSession": "+ 新会话",
  "app.session": "会话 {id}",
  "app.newSessionLabel": "新会话",
  "app.connecting": "连接中…",
  "app.busConnected": "总线已连接",
  "app.busUnreachable": "总线不可达",
  "app.selectModel": "选择模型",
  "app.model": "模型:",
  "app.tokens": "{used} / {context} 令牌",
  "app.switchLight": "切换到亮色模式",
  "app.switchDark": "切换到暗色模式",
  "app.switchLanguage": "切换语言",
  "app.browserNotice": "当前在浏览器中运行 — 总线桥接只存在于桌面外壳中。请用以下命令启动",
  "app.unreachableNotice": "无法连接 {url} 上的总线 — 请启动 harness 并保持其终端开启：",
  "app.conversationHeld": "此会话已在 Niffler {n} 中打开。",
  "app.conversationHeldOther": "此会话已在另一个 UI 中打开。",
  "app.conversationHeldSwitched": "此会话已在 Niffler {n} 中打开 — 已开始新会话。",
  "app.conversationHeldOtherSwitched": "此会话已在另一个 UI 中打开 — 已开始新会话。",
  "app.startFresh": "开始新会话",
  "app.dismiss": "关闭",
  "app.approvalRequired": "需要批准",
  "app.approvalWaiting": "一个带 x-harness.approval 的工具调用正在等待你的确认：",
  "app.moreWaiting": "还有 {n} 个正在等待",
  "app.autoApproving": "本会话自动批准：",
  "app.enterEsc": "回车批准 · Esc 拒绝",
  "app.deny": "拒绝",
  "app.approve": "批准",
  "app.autoApprove": "自动批准",
  "app.autoApproveTitle": "批准，且本会话内不再询问该工具",
  "app.continueTitle": "继续下去？",
  "app.continueBody": "本回合已触及{dimension}上限：",
  "app.continueHint": "继续会把该上限提高一档并让回合跑下去 · 回车继续 · Esc 停止",
  "app.continueYes": "继续",
  "app.continueNo": "停止",
  "app.dimensionRounds": "轮数",
  "app.dimensionTokens": "令牌",
  "app.dimensionSeconds": "时长",
  "sessions.newSession": "新会话",
  "sessions.notStarted": "尚未开始",
  "sessions.rename": "重命名",
  "sessions.delete": "删除会话",
  "sessions.confirmDelete": "再次点击以删除",
  "sessions.sure": "确定?",
  "chat.placeholder": "让 Niffler 做点什么…",
  "model.search": "搜索模型…",
  "model.customId": "自定义模型 ID",
  "model.set": "设置",
  "model.useDefault": "使用提供商默认",
  "model.clearsOverride": "（清除覆盖）",
  "model.searching": "搜索中…",
  "model.noneFound": "未找到模型",
  "model.noneFoundFor": "未找到与 \"{catalog}\" 匹配的模型",
  "model.reason": "推理",
  "model.tools": "工具",
  "model.showing": "显示 {n} / {total} — 请缩小搜索范围",
  "pm.title": "提供商",
  "pm.addProvider": "添加提供商",
  "pm.editProvider": "编辑提供商",
  "pm.save": "保存",
  "pm.saving": "保存中…",
  "pm.edit": "编辑",
  "pm.sure": "确定？",
  "pm.remove": "移除",
  "pm.active": "激活",
  "pm.noKey": "无密钥",
  "mcp.title": "MCP 服务器",
  "mcp.addServer": "添加服务器",
  "mcp.editServer": "编辑服务器",
  "mcp.save": "保存",
  "mcp.saving": "正在验证并保存…",
  "mcp.edit": "编辑",
  "mcp.sure": "确定？",
  "mcp.remove": "移除",
  "mcp.refresh": "刷新",
  "mcp.live": "运行中",
  "mcp.notRunning": "未运行",
  "mcp.disabled": "已停用",
  "mcp.enabled": "启用",
  "mcp.validateNow": "立即验证并启动",
  "mcp.loading": "加载中…",
  "mcp.none": "尚未配置 MCP 服务器",
  "mcp.tools": "{n} 个工具",
  "mcp.keepBlank": "（留空保留）",
  "mcp.hint": "工具以 mcp_<server>_<tool> 形式出现，通过 discover + invoke 调用。保存时会真实连接验证一次——npx/uvx 服务器首次运行可能需要下载依赖。",
  "pc.provider": "提供商",
  "pc.providerDetail": "提供商：{provider}（{source}）",
  "components.direct": "直接",
  "components.seen": "已见",
  "components.demand": "按需",
  "components.internal": "内部",
  "components.noneSeen": "尚未见过",
  "components.exposureInit": "工具暴露在首个回合后初始化",
  "components.noSession": "开始或选择会话以查看工具暴露",
  "components.tools": "工具",
  "app.effort": "effort",
  "app.think": "think",
  "app.toolsDisplay": "tool",
  "app.effortTitle": "思考强度（回合间生效）",
  "app.thinkTitle": "推理显示",
  "app.toolsTitle": "工具卡片显示",
  "app.stop": "停止",
  "app.stopConfirm": "停止？",
  "app.stopping": "停止中…",
  "chat.send": "发送",
  "chat.steer": "引导",
  "chat.sendWhileBusy": "回合进行中 — 回车将引导它",
  "chat.unknownCommand": "未知命令：/{name}",
  "chat.thinkingEffort": "思考强度：{level}",
  "chat.thinkLevel": "推理显示：{level}",
  "chat.toolLevel": "工具卡片显示：{level}",
  "chat.slashExec": "→ /{name} {args}",
  "chat.slashFailed": "/{name} 失败：{err}",
  "chat.completing": "获取 {token}…",
  "chat.levelAuto": "自动",
  "chat.levelFull": "完整",
  "chat.levelBrief": "简要",
  "chat.levelOff": "关闭",
  "chat.thinkingCollapsed": "▸ 思考中…",
  "chat.contextAt": "上下文占用 {pct}% — 将在 {trim}% 时压缩/裁剪",
  "chat.contextAtSoon": "上下文占用 {pct}% — 即将压缩/裁剪",
  "chat.compacted": "上下文已压缩 — 显示的大小是估算值，下一次请求会重新测量",
  "chat.contextTrimmed": "上下文已裁剪 — 丢弃了 {n} 条较早消息",
  "chat.pruned": "工具结果已修剪 — 回收 {bytes} 字节",
  "chat.cacheReset": "历史已裁剪 — 下一次请求将重建提供商提示词缓存",
  "chat.compactionUnavailable": "压缩不可用",
  "chat.compactionDeclined": "压缩被拒绝",
  "chat.compactionInvalid": "压缩候选被驳回",
  "chat.compactionStale": "压缩已过期",
  "chat.compactionFailed": "压缩失败",
  "chat.compactionOther": "压缩 {reason}",
  "status.provider": "供应商",
  "status.unknown": "未知",
  "status.unknownSource": "未知来源",
  "status.none": "无",
  "status.env": "（环境）",
  "status.global": "（全局）",
  "status.session": "（会话）",
  "status.ctxNone": "无上下文限制",
  "status.ctxEmpty": "上限 {context}",
  "status.detailTitle": "供应商/模型/上下文详情",
  "status.detailProvider": "供应商：{provider}（{source}）",
  "status.detailModel": "模型：{model}",
  "status.detailCatalog": "目录：{catalog}",
  "status.detailContext": "上下文上限：{context}（{source}）",
  "status.detailOutput": "输出上限：{output}（{source}）",
  "status.detailUsed": "上下文已用：{used}（{pct}%）",
  "status.detailCache": "提示缓存：{read}/{prompt}（{rate}%）",
  "status.detailStrip": "模型前缀剥离：开",
  "status.detailOverride": "模型覆写：{model}",
  "help.title": "命令：",
  "help.keys": "Ctrl+T 推理 · Ctrl+E 工具卡片 · Ctrl+G 强度",
  "help.pluginTitle": "插件命令：",
  "slash.components": "运行中的组件与会话工具暴露",
  "slash.discover": "将组件 schema 追加到本对话（tool=NAME 指定单个工具）",
  "slash.profile": "为新对话选择工具 profile；default 清除",
  "slash.provider": "选择全局供应商",
  "slash.model": "选择本对话的模型",
  "slash.effort": "思考强度",
  "slash.connect": "打开供应商设置",
  "slash.status": "供应商/模型/上下文详情",
  "slash.new": "开始新对话",
  "slash.session": "切换对话",
  "slash.think": "推理显示：full、brief 或 off",
  "slash.tools": "工具卡片显示：brief、full 或 off",
  "slash.locale": "切换界面语言",
  "slash.info": "本会话统计：模型、消息、令牌与提示缓存",
  "slash.help": "显示帮助",
  "slash.approvals": "本对话的批准关卡：ask（默认）或 auto",
  "slash.limit": "本对话的软性回合上限（rounds/tokens/seconds）；clear 清除",
  "approvals.current": "批准关卡：{mode}",
  "approvals.updated": "批准关卡已设置：{mode}",
  "approvals.modeAsk": "ask — 需要批准的工具调用会询问你",
  "approvals.modeAuto": "auto — 需要批准的工具调用直接执行",
  "limits.current": "软性回合上限：{limits}",
  "limits.updated": "软性回合上限：{limits} — 未指定的维度会被清除",
  "limits.cleared": "软性回合上限已清除 — 只剩硬性预算",
  "limits.none": "无",
  "limits.rounds": "{n} 轮",
  "limits.tokens": "{n} 令牌",
  "limits.seconds": "{n} 秒",
  "limits.clearArgs": "clear 不接受参数",
  "limits.hint": "触及任一上限时会询问是否继续本回合",
  "controls.noSession": "未选择对话 — 先用 /new 新建或先发送一条消息",
  "info.id": "ID：{id}",
  "info.title": "标题：{title}",
  "info.model": "模型：{model} · {provider}",
  "info.effort": "思考强度：{level}",
  "info.created": "创建：{time}",
  "info.cwd": "工作目录：{cwd}",
  "info.messages": "消息：{total}（用户 {user} · 助手 {assistant} · 工具 {tool}）",
  "info.context": "上下文：{used} / {limit}（{pct}%）",
  "info.contextUsed": "上下文：{used}",
  "info.input": "输入：{total} — 缓存 {cached}（{rate}%）· 未缓存 {uncached}",
  "info.output": "输出：{output}",
  "info.noSession": "未选择会话 — /info [id] 可查看其他会话",
};

const zhTW: Record<DictKey, string> = {
  "app.newSession": "+ 新對話",
  "app.session": "對話 {id}",
  "app.newSessionLabel": "新對話",
  "app.connecting": "連線中…",
  "app.busConnected": "匯流排已連線",
  "app.busUnreachable": "匯流排無法連線",
  "app.selectModel": "選擇模型",
  "app.model": "模型:",
  "app.tokens": "{used} / {context} 詞元",
  "app.switchLight": "切換到亮色模式",
  "app.switchDark": "切換到暗色模式",
  "app.switchLanguage": "切換語言",
  "app.browserNotice": "目前於瀏覽器中執行 — 匯流排橋接只存在於桌面外殼中。請用以下指令啟動",
  "app.unreachableNotice": "無法連線到 {url} 的匯流排 — 請啟動 harness 並保持其終端機開啟：",
  "app.conversationHeld": "此對話已在 Niffler {n} 中開啟。",
  "app.conversationHeldOther": "此對話已在另一個 UI 中開啟。",
  "app.conversationHeldSwitched": "此對話已在 Niffler {n} 中開啟 — 已開始新對話。",
  "app.conversationHeldOtherSwitched": "此對話已在另一個 UI 中開啟 — 已開始新對話。",
  "app.startFresh": "開始新對話",
  "app.dismiss": "關閉",
  "app.approvalRequired": "需要核准",
  "app.approvalWaiting": "一個帶有 x-harness.approval 的工具呼叫正在等待你的確認：",
  "app.moreWaiting": "還有 {n} 個正在等待",
  "app.autoApproving": "本對話自動核准：",
  "app.enterEsc": "Enter 核准 · Esc 拒絕",
  "app.deny": "拒絕",
  "app.approve": "核准",
  "app.autoApprove": "自動核准",
  "app.autoApproveTitle": "核准，且本對話內不再詢問該工具",
  "app.continueTitle": "要繼續嗎？",
  "app.continueBody": "本回合已觸及{dimension}上限：",
  "app.continueHint": "繼續會把該上限提高一階並讓回合跑下去 · Enter 繼續 · Esc 停止",
  "app.continueYes": "繼續",
  "app.continueNo": "停止",
  "app.dimensionRounds": "回合數",
  "app.dimensionTokens": "詞元",
  "app.dimensionSeconds": "時間",
  "sessions.newSession": "新對話",
  "sessions.notStarted": "尚未開始",
  "sessions.rename": "重新命名",
  "sessions.delete": "刪除對話",
  "sessions.confirmDelete": "再點一次以刪除",
  "sessions.sure": "確定?",
  "chat.placeholder": "讓 Niffler 做點什麼…",
  "model.search": "搜尋模型…",
  "model.customId": "自訂模型 ID",
  "model.set": "設定",
  "model.useDefault": "使用供應商預設值",
  "model.clearsOverride": "（清除覆寫）",
  "model.searching": "搜尋中…",
  "model.noneFound": "找不到模型",
  "model.noneFoundFor": "找不到符合 \"{catalog}\" 的模型",
  "model.reason": "推理",
  "model.tools": "工具",
  "model.showing": "顯示 {n} / {total} — 請縮小搜尋範圍",
  "pm.title": "供應商",
  "pm.addProvider": "新增供應商",
  "pm.editProvider": "編輯供應商",
  "pm.save": "儲存",
  "pm.saving": "儲存中…",
  "pm.edit": "編輯",
  "pm.sure": "確定？",
  "pm.remove": "移除",
  "pm.active": "啟用",
  "pm.noKey": "無金鑰",
  "mcp.title": "MCP 伺服器",
  "mcp.addServer": "新增伺服器",
  "mcp.editServer": "編輯伺服器",
  "mcp.save": "儲存",
  "mcp.saving": "正在驗證並儲存…",
  "mcp.edit": "編輯",
  "mcp.sure": "確定？",
  "mcp.remove": "移除",
  "mcp.refresh": "重新整理",
  "mcp.live": "執行中",
  "mcp.notRunning": "未執行",
  "mcp.disabled": "已停用",
  "mcp.enabled": "啟用",
  "mcp.validateNow": "立即驗證並啟動",
  "mcp.loading": "載入中…",
  "mcp.none": "尚未設定 MCP 伺服器",
  "mcp.tools": "{n} 個工具",
  "mcp.keepBlank": "（留空保留）",
  "mcp.hint": "工具以 mcp_<server>_<tool> 形式出現，透過 discover + invoke 呼叫。儲存時會真實連線驗證一次——npx/uvx 伺服器首次執行可能需要下載相依套件。",
  "pc.provider": "供應商",
  "pc.providerDetail": "供應商：{provider}（{source}）",
  "components.direct": "直接",
  "components.seen": "已見",
  "components.demand": "按需",
  "components.internal": "內部",
  "components.noneSeen": "尚未見過",
  "components.exposureInit": "工具暴露在第一回合後初始化",
  "components.noSession": "開始或選擇對話以查看工具暴露",
  "components.tools": "工具",
  "app.effort": "effort",
  "app.think": "think",
  "app.toolsDisplay": "tool",
  "app.effortTitle": "思考強度（回合間生效）",
  "app.thinkTitle": "推理顯示",
  "app.toolsTitle": "工具卡片顯示",
  "app.stop": "停止",
  "app.stopConfirm": "停止？",
  "app.stopping": "停止中…",
  "chat.send": "傳送",
  "chat.steer": "引導",
  "chat.sendWhileBusy": "回合進行中 — Enter 將引導它",
  "chat.unknownCommand": "未知命令：/{name}",
  "chat.thinkingEffort": "思考強度：{level}",
  "chat.thinkLevel": "推理顯示：{level}",
  "chat.toolLevel": "工具卡片顯示：{level}",
  "chat.slashExec": "→ /{name} {args}",
  "chat.slashFailed": "/{name} 失敗：{err}",
  "chat.completing": "取得 {token}…",
  "chat.levelAuto": "自動",
  "chat.levelFull": "完整",
  "chat.levelBrief": "簡要",
  "chat.levelOff": "關閉",
  "chat.thinkingCollapsed": "▸ 思考中…",
  "chat.contextAt": "上下文佔用 {pct}% — 將在 {trim}% 時壓縮/裁剪",
  "chat.contextAtSoon": "上下文佔用 {pct}% — 即將壓縮/裁剪",
  "chat.compacted": "上下文已壓縮 — 顯示的大小是估算值，下一次請求會重新測量",
  "chat.cacheReset": "歷史已裁剪 — 下一次請求將重建提供商提示詞快取",
  "chat.contextTrimmed": "上下文已裁剪 — 丟棄了 {n} 條較早訊息",
  "chat.pruned": "工具結果已修剪 — 回收 {bytes} 位元組",
  "chat.compactionUnavailable": "壓縮不可用",
  "chat.compactionDeclined": "壓縮被拒絕",
  "chat.compactionInvalid": "壓縮候選被駁回",
  "chat.compactionStale": "壓縮已過期",
  "chat.compactionFailed": "壓縮失敗",
  "chat.compactionOther": "壓縮 {reason}",
  "status.provider": "供應商",
  "status.unknown": "未知",
  "status.unknownSource": "未知來源",
  "status.none": "無",
  "status.env": "（環境）",
  "status.global": "（全域）",
  "status.session": "（對話）",
  "status.ctxNone": "無上下文限制",
  "status.ctxEmpty": "上限 {context}",
  "status.detailTitle": "供應商/模型/上下文詳情",
  "status.detailProvider": "供應商：{provider}（{source}）",
  "status.detailModel": "模型：{model}",
  "status.detailCatalog": "目錄：{catalog}",
  "status.detailContext": "上下文上限：{context}（{source}）",
  "status.detailOutput": "輸出上限：{output}（{source}）",
  "status.detailUsed": "上下文已用：{used}（{pct}%）",
  "status.detailCache": "提示快取：{read}/{prompt}（{rate}%）",
  "status.detailStrip": "模型前綴剝離：開",
  "status.detailOverride": "模型覆寫：{model}",
  "help.title": "命令：",
  "help.keys": "Ctrl+T 推理 · Ctrl+E 工具卡片 · Ctrl+G 強度",
  "help.pluginTitle": "外掛命令：",
  "slash.components": "執行中的元件與對話工具暴露",
  "slash.discover": "將元件 schema 附加到本對話（tool=NAME 指定單一工具）",
  "slash.profile": "為新對話選擇工具 profile；default 清除",
  "slash.provider": "選擇全域供應商",
  "slash.model": "選擇本對話的模型",
  "slash.effort": "思考強度",
  "slash.connect": "開啟供應商設定",
  "slash.status": "供應商/模型/上下文詳情",
  "slash.new": "開始新對話",
  "slash.session": "切換對話",
  "slash.think": "推理顯示：full、brief 或 off",
  "slash.tools": "工具卡片顯示：brief、full 或 off",
  "slash.locale": "切換介面語言",
  "slash.info": "本對話統計：模型、訊息、詞元與提示快取",
  "slash.help": "顯示說明",
  "slash.approvals": "本對話的核准關卡：ask（預設）或 auto",
  "slash.limit": "本對話的軟性回合上限（rounds/tokens/seconds）；clear 清除",
  "approvals.current": "核准關卡：{mode}",
  "approvals.updated": "核准關卡已設定：{mode}",
  "approvals.modeAsk": "ask — 需要核准的工具呼叫會詢問你",
  "approvals.modeAuto": "auto — 需要核准的工具呼叫直接執行",
  "limits.current": "軟性回合上限：{limits}",
  "limits.updated": "軟性回合上限：{limits} — 未指定的維度會被清除",
  "limits.cleared": "軟性回合上限已清除 — 只剩硬性預算",
  "limits.none": "無",
  "limits.rounds": "{n} 輪",
  "limits.tokens": "{n} 詞元",
  "limits.seconds": "{n} 秒",
  "limits.clearArgs": "clear 不接受參數",
  "limits.hint": "觸及任一上限時會詢問是否繼續本回合",
  "controls.noSession": "未選擇對話 — 先用 /new 新增或先傳送一則訊息",
  "info.id": "ID：{id}",
  "info.title": "標題：{title}",
  "info.model": "模型：{model} · {provider}",
  "info.effort": "思考強度：{level}",
  "info.created": "建立：{time}",
  "info.cwd": "工作目錄：{cwd}",
  "info.messages": "訊息：{total}（使用者 {user} · 助理 {assistant} · 工具 {tool}）",
  "info.context": "上下文：{used} / {limit}（{pct}%）",
  "info.contextUsed": "上下文：{used}",
  "info.input": "輸入：{total} — 快取 {cached}（{rate}%）· 未快取 {uncached}",
  "info.output": "輸出：{output}",
  "info.noSession": "未選擇對話 — /info [id] 可檢視其他對話",
};

const DICTS: Record<Locale, Record<DictKey, string>> = { en, zh, "zh-TW": zhTW };
const STORAGE_KEY = "niffler-lang";

function detect(): Locale {
  try {
    const saved = localStorage.getItem(STORAGE_KEY);
    if (saved === "en" || saved === "zh" || saved === "zh-TW") return saved;
  } catch {
    // localStorage unavailable — fall through to navigator.language
  }
  const nav = (navigator.language || "en").toLowerCase();
  if (nav.startsWith("zh")) {
    return nav.includes("tw") || nav.includes("hant") || nav.includes("hk")
      ? "zh-TW"
      : "zh";
  }
  return "en";
}

let current: Locale = $state(detect());

export function locale(): Locale {
  return current;
}

export function t(key: DictKey, vars?: Record<string, string>): string {
  const s = DICTS[current][key];
  if (!vars) return s;
  return s.replace(/\{(\w+)\}/g, (_, k: string) => vars[k] ?? `{${k}}`);
}

/** Localized description of a built-in slash command. Translations live
 * under `slash.<name>`; a builtin without one (newly added) falls back to
 * its English declaration in slash.ts — the command list never depends on
 * translation completeness. */
export function slashDescription(name: string, fallback: string): string {
  return DICTS[current][("slash." + name) as DictKey] ?? fallback;
}

export function setLocale(l: Locale): void {
  current = l;
  try {
    localStorage.setItem(STORAGE_KEY, l);
  } catch {
    // non-fatal
  }
}

export function cycleLocale(): void {
  current = current === "en" ? "zh" : current === "zh" ? "zh-TW" : "en";
  try {
    localStorage.setItem(STORAGE_KEY, current);
  } catch {
    // non-fatal: the choice just won't survive a reload
  }
}
